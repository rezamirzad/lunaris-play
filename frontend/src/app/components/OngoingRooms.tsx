...
export default function OngoingRooms({ rooms, onJoin, onViewBoard, t }: OngoingRoomsProps) {
          ...
          >
              ...
              <div className="flex flex-col min-w-0">
                <div className="flex items-center gap-3">
                  ...
                  {isFinished && winnerName && (
                    <div className="flex items-center gap-1.5 bg-teal-400/10 border border-teal-400/20 px-2 py-0.5 rounded-lg shrink-0">
                      <span className="text-[10px]">🏆</span>
                      <span className="text-[9px] font-black italic text-teal-400 uppercase truncate max-w-[80px]">
                        {winnerName}
                      </span>
                    </div>
                  )}
            ...
            <div className="grid grid-cols-2 gap-2">
              <motion.button
                ...
                className="bg-zinc-900/50 border border-white/5 text-zinc-400 py-2.5 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all"
              >
                Board
              </motion.button>
              <motion.button
                whileHover={!isFinished ? { scale: 1.02, backgroundColor: "rgba(45,212,191,0.1)" } : {}}
                whileTap={!isFinished ? { scale: 0.98 } : {}}
                onClick={() => !isFinished && onJoin(room.roomCode)}
                disabled={isFinished}
                className={`py-2.5 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all ${isFinished ? "bg-zinc-950 text-zinc-700 cursor-not-allowed border border-white/5" : "bg-teal-400/10 border border-teal-400/30 text-teal-400 hover:text-white"}`}
              >

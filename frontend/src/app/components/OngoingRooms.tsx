...
export default function OngoingRooms({
  ...
  if (!rooms || rooms.length === 0) {
    return (
      ...
      >
        <p className={`text-zinc-600 font-bold uppercase text-[9px] ${lang === 'fa' ? 'tracking-normal' : 'tracking-[0.3em]'}`}>
          {t.noOngoing}
        </p>
      </motion.div>
    );
  ...
  return (
    <div className="max-h-[40vh] lg:max-h-[720px] overflow-y-auto pr-2 custom-scrollbar">
      <div className="space-y-3 font-mono pb-2">
            ...
            >
              ...
              <div className="grid grid-cols-2 gap-2">
                <motion.button
                  ...
                  className={`bg-zinc-900/50 border border-white/2 py-2.5 rounded-xl text-[12px] font-black uppercase transition-all ${lang === 'fa' ? 'tracking-normal' : 'tracking-widest'}`}
                >
                  {t.board}
                </motion.button>
                <motion.button
                  ...
                  disabled={isFinished}
                  className={`py-2.5 rounded-xl text-[12px] font-black uppercase transition-all ${isFinished ? "bg-zinc-950 text-zinc-500 cursor-not-allowed border border-white/5" : "bg-teal-400/10 border border-teal-400/30 text-teal-400 hover:text-white"} ${lang === 'fa' ? 'tracking-normal' : 'tracking-widest'}`}
                >

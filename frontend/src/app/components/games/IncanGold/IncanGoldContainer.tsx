...
const IncanGoldBoard: React.FC<BoardProps> = ({ roomId, roomData }) => {
  ...
  return (
    <div className="flex flex-col h-full w-full bg-[#0d0700] text-amber-50 p-6 overflow-hidden relative font-serif">
      ...
      <div
        className="absolute inset-0 pointer-events-none border-[2rem] border-[#1a0f00] opacity-40 z-0"
        style={{
          clipPath:
            "polygon(0% 0%, 100% 0%, 100% 100%, 0% 100%, 0% 0%, 4% 4%, 4% 96%, 96% 96%, 96% 4%, 4% 4%)",
        }}
      />
      ...
      <header className="flex justify-between items-start mb-8 z-10 px-4">
        <div className="relative">
          <motion.h1
            ...
            className="text-7xl font-black text-transparent bg-clip-text bg-gradient-to-b from-amber-100 via-amber-400 to-amber-700 tracking-tighter filter drop-shadow-[0_2px_15px_rgba(245,158,11,0.5)] italic"
          >
            DIAMANT
          </motion.h1>
          <div className="flex items-center gap-4 mt-2">
            <div className="flex items-center gap-2 bg-black/60 border border-amber-500/30 px-4 py-1 rounded-full shadow-inner">
            ...
            <button
              ...
              className="group flex items-center gap-2 text-amber-500/60 hover:text-amber-400 transition-all text-xs font-bold uppercase tracking-[0.2em] italic border-b border-amber-500/10 hover:border-amber-500/40 pb-0.5"
            >
          ...
          {players.map((p) => {
            ...
            const roundResults =
              board.roundHistory?.[board.roundHistory.length - 1]
                ?.playerResults?.[p._id];
            const crashed =
              roundResults?.status === "CRASHED" &&
              board.phase === "ROUND_RESULTS";
            const hasDecided = board.decisions[p._id];
            const choiceRevealed =
              board.phase === "VOTE_REVEAL" || board.phase === "ROUND_RESULTS";
            ...
            return (
              <motion.div
                key={p._id}
                layout
                className={`min-w-[210px] h-full rounded-[2.5rem] border-2 flex flex-col p-5 transition-all duration-700 relative overflow-hidden group ${hasDecided && !choiceRevealed ? "border-amber-400 shadow-[0_0_30px_rgba(245,158,11,0.4)] bg-amber-500/10" : isInCave ? "bg-emerald-950/20 border-emerald-500/40 shadow-[0_0_30px_rgba(16,185,129,0.1)]" : "bg-[#0a0500]/60 border-white/5 opacity-50"}`}
              >
                {hasDecided && !choiceRevealed && (
                ...
                <div className="flex items-center justify-between mb-3 z-10 relative text-left">
                  <div className="flex items-center gap-2.5 min-w-0">
                    <div
                      className={`w-10 h-10 rounded-2xl flex items-center justify-center text-xl bg-black/50 border-2 border-white/5 shadow-inner transition-transform duration-500 ${isInCave ? "animate-bounce-subtle" : "scale-90 opacity-60"}`}
                    >
                      {choiceRevealed
                        ? choice === "LEAVE"
                          ? "⛺"
                    ...
                    <div className="flex flex-col min-w-0">
                      <span
                        className={`text-[11px] font-black uppercase tracking-tight truncate ${isInCave ? "text-emerald-400" : "text-slate-400"}`}
                      >
                        {p.name}
                      </span>
                      <div className="flex items-center gap-1">
                        <div
                          className={`w-1.5 h-1.5 rounded-full ${isInCave ? "bg-emerald-500 animate-pulse" : "bg-slate-700"}`}
                        />
                        <span className="text-[7px] font-bold opacity-40 uppercase tracking-widest truncate">
                          {isInCave
                            ? t.incangold_in_the_cave
                            : t.incangold_at_base_camp}
                        </span>
                      </div>
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                    {board.phase === "DECISION_PHASE" && isInCave && (
                      <motion.div
                        initial={{ scale: 0 }}
                        animate={{ scale: 1 }}
                        className={`w-8 h-8 rounded-full border-2 flex items-center justify-center transition-all duration-500 ${hasDecided ? "bg-emerald-500 border-emerald-300 shadow-[0_0_15px_rgba(16,185,129,0.6)]" : "bg-slate-800/50 border-slate-700"}`}
                      >
                        {hasDecided && (
                          <motion.span
                            initial={{ y: 5 }}
                    ...
                    {choiceRevealed && isInCave && (
                      <motion.div
                        ...
                        className={`px-3 py-1 rounded-xl text-[10px] font-black uppercase tracking-[0.2em] shadow-lg ${choice === "LEAVE" ? "bg-amber-500 text-amber-950 border-t border-amber-300/50" : "bg-emerald-500 text-emerald-950 border-t border-emerald-300/50"}`}
                      >
                        {choice === "LEAVE" ? "Camp" : "Torch"}
                      </motion.div>
                    )}
                  </div>
                </div>
                <div className="flex-1 flex flex-col justify-end gap-2 z-10 relative">
                  <div
                    className={`flex justify-between items-center bg-black/60 px-3.5 py-2 rounded-xl border border-white/5 transition-all ${isInCave ? "border-amber-500/20" : "opacity-40"}`}
                  >
                    ...
                    <div className="flex items-center gap-1.5">
                      <span
                        className={`text-base font-black tabular-nums ${crashed ? "text-red-500 line-through opacity-50" : "text-amber-400"}`}
                      >
                        {state.gemsThisRound}
                      </span>
                  ...
                  <div className="flex justify-between items-center bg-black/60 px-3.5 py-2 rounded-xl border border-white/5 transition-all hover:border-emerald-500/20">
                    {/* Left: Label and Icon */}
                    <div className="flex items-center gap-1.5">
                ...
                {crashed && (
                  <motion.div
                    initial={{ opacity: 0 }}
                    animate={{ opacity: 1 }}
                    className="absolute inset-0 bg-red-950/90 rounded-[2rem] flex flex-col items-center justify-center text-white backdrop-blur-[1px] z-20 border-2 border-red-500/50"
                  >
                    ...
                    <span className="text-[10px] font-black uppercase tracking-[0.2em] italic text-red-100">
                      {formatLog(t.incangold_lost_gems, {
                        n: roundResults.lost,
                      })}
                    </span>
                  </motion.div>
      ...
      <AnimatePresence>
        {showRules && (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="absolute inset-0 bg-black/98 z-[60] p-16 overflow-y-auto backdrop-blur-2xl flex items-center justify-center"
          >
            <div className="max-w-5xl w-full bg-[#1a0f00] border-8 border-amber-900/40 rounded-[4rem] p-16 relative shadow-[0_0_100px_rgba(0,0,0,1)] overflow-hidden">
              ...
              <div className="flex justify-between items-center mb-16 relative z-10">
                <div className="flex items-center gap-6">
                  ...
                  <h2 className="text-7xl font-black text-transparent bg-clip-text bg-gradient-to-b from-amber-200 to-amber-600 tracking-tighter italic uppercase leading-none">
                    {t.incangold_rules_title}
                  </h2>
                </div>
                <button
                  onClick={() => setShowRules(false)}
                  className="bg-amber-900/20 hover:bg-amber-500 hover:text-amber-950 text-amber-500 w-16 h-14 rounded-2xl transition-all text-4xl flex items-center justify-center font-black"
                >
                  ×
                </button>
              </div>
              <div className="grid grid-cols-2 gap-16 text-slate-300 leading-relaxed relative z-10">
                <section className="space-y-10">
                  <div className="bg-white/5 p-8 rounded-[2.5rem] border-l-4 border-amber-500/40">
                    <h4 className="text-amber-400 text-2xl font-black uppercase tracking-widest mb-4 italic">
                      {t.incangold_goal_title}
                    </h4>
                    <p className="text-lg opacity-80">
                      {t.incangold_goal_desc}
                    </p>
                  </div>
                  <div className="bg-white/5 p-8 rounded-[2.5rem] border-l-4 border-amber-500/40">
                    <h4 className="text-amber-400 text-2xl font-black uppercase tracking-widest mb-4 italic">
                      {t.incangold_gems_rules_title}
                    </h4>
                    <p className="text-lg opacity-80">
                      {t.incangold_gems_rules_desc}
                    </p>
                  </div>
                  <div className="bg-red-500/5 p-8 rounded-[2.5rem] border-l-4 border-red-500/40">
                    <h4 className="text-red-400 text-2xl font-black uppercase tracking-widest mb-4 italic">
                      {t.incangold_dangers_rules_title}
                    </h4>
                    <p className="text-lg opacity-80">
                      {t.incangold_dangers_rules_desc}
                    </p>
                  </div>
                </section>
                <section className="space-y-10">
                  <div className="bg-emerald-500/5 p-8 rounded-[2.5rem] border-l-4 border-emerald-500/40">
                    <h4 className="text-emerald-400 text-2xl font-black uppercase tracking-widest mb-4 italic">
                      {t.incangold_dilemma_rules_title}
                    </h4>
                    <p className="text-lg opacity-80">
                      {t.incangold_dilemma_rules_desc}
                    </p>
                  </div>
                  <div className="bg-white/5 p-8 rounded-[2.5rem] border-l-4 border-amber-500/40">
                    <h4 className="text-amber-400 text-2xl font-black uppercase tracking-widest mb-4 italic">
                      {t.incangold_leaving_rules_title}
                    </h4>
                    <p className="text-lg opacity-80">
                      {t.incangold_leaving_rules_desc}
                    </p>
                  </div>
                  <div className="bg-white/5 p-8 rounded-[2.5rem] border-l-4 border-amber-500/40">
                    <h4 className="text-amber-400 text-2xl font-black uppercase tracking-widest mb-4 italic">
                      {t.incangold_safety_rules_title}
                    </h4>
                    <p className="text-lg opacity-80">
                      {t.incangold_safety_rules_desc}
                    </p>
                  </div>
                </section>
              </div>

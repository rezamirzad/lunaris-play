...
export default function JustOneBoard({ roomData }: BoardProps) {
  ...
  const missionMysteryWord = mysteryWordObj[board.language] || mysteryWordObj["en"];

  // Filter valid clues
  const validClues = Object.entries(board.clues || {})
    .filter(([pId]) => !board.canceledClues.includes(pId as Doc<"players">["_id"]))
    .map(([, clue]) => clue);
          ...
          <section>
            ...
            <div className="flex flex-col gap-4">
              {players.map((player) => {
                ...
                const hasConfirmed = board.confirmedPlayers?.includes(player._id);
                const lenientVotesObj = (board.lenientVotes || {}) as Record<string, boolean>;
                    ...
                    {!isInfiltrator && (
                      <div className="flex flex-col gap-2 mt-2">
                        <div className="flex items-center gap-2">
                          <div className={`h-1.5 w-1.5 rounded-full ${hasSubmitted ? "bg-teal-400 shadow-[0_0_8px_rgba(45,212,191,0.6)]" : "bg-zinc-800"}`} />
                          <span className="text-[7px] font-black text-zinc-500 uppercase tracking-widest">
                            {hasSubmitted ? "DATA_LINKED" : "UPLINK_PENDING..."}
                          </span>
                        </div>
                        {board.phase === "VALIDATION" && hasConfirmed && (
                           ...
                           >
                              ...
                              <span className="text-[6px] font-black text-teal-400 uppercase tracking-widest">VALIDATED</span>
                           </motion.div>
                        )}
                        {isLenient && lenientVote !== undefined && (
                           ...
                           >
                              <div className={`w-1 h-1 rounded-full ${lenientVote ? "bg-teal-400" : "bg-rose-400"} animate-pulse`} />
                              <span className="text-[6px] font-black uppercase tracking-widest">{lenientVote ? "ACCEPTED" : "REJECTED"}</span>
                           </motion.div>
                        )}
                      </div>
                    )}
        ...
        <div className="lg:col-span-6 flex flex-col items-center justify-center space-y-12 py-12">
          ...
          <div className="w-full flex justify-between px-12 mb-8">
             <div className="flex flex-col gap-1">
                <span className="text-[8px] font-black text-zinc-600 uppercase tracking-[0.4em]">CURRENT_SESSION_SCORE</span>
             ...
             <div className="flex flex-col items-end gap-1">
                <span className="text-[8px] font-black text-zinc-600 uppercase tracking-[0.4em]">MISSION_ROUND</span>
          ...
          <AnimatePresence mode="wait">
             {isLenient ? (
               ...
               >
                  ...
                  <div className="space-y-4">
                     <span className="text-[10px] font-bold text-zinc-500 tracking-[0.3em] uppercase">TARGET_ENCRYPTION_KEY</span>
                  ...
                  <div className="px-8 py-3 bg-white/5 border border-white/10 rounded-full animate-pulse">
                     <span className="text-[10px] font-black text-zinc-400 tracking-[0.2em] uppercase">HACKER_CONSENSUS_REQUIRED</span>
                  </div>
               </motion.div>
             ) : !isGuessing ? (
               <motion.div
               ...
               >
                  <span className="text-[10px] text-zinc-500 font-black tracking-[0.8em] uppercase">ENCRYPTED_TARGET_NODE</span>
                  
                  {/* Aura's Masked Letter Blocks */}
                  <div className="flex gap-4">
                     {missionMysteryWord.split("").map((char: string, i: number) => (
                       <div key={i} className="w-12 h-16 border-b-4 border-zinc-800 flex items-center justify-center font-black text-4xl text-white">
                          <motion.span
                            animate={isRoundResults ? { opacity: 1 } : { opacity: [0.2, 0.4, 0.2] }}
                  ...
                  {!isRoundResults && !isGameOver && (
                    <div className="mt-4 px-6 py-2 bg-teal-500/10 border border-teal-500/20 rounded-lg animate-pulse">
                       <span className="text-[10px] font-black text-teal-400 tracking-[0.3em] uppercase">WAITING FOR HACKER CLUES...</span>
                    </div>
                  )}
               </motion.div>
             ) : (
               ...
               >
                  ...
                  {validClues.length === 0 && (
                    <div className="flex flex-col items-center gap-4 py-12 opacity-40">
                       ...
                       <span className="text-[10px] font-black tracking-[0.5em] text-rose-500">ALL_CLUES_CANCELED_BY_PROTOCOL</span>
                    </div>
                  )}
               </motion.div>
             )}
          ...
          <div className="mt-12 text-[8px] text-zinc-700 font-black tracking-[0.5em] uppercase">
             SESSION_STATUS: {board.phase.replace("_", " ")}
          </div>
        </div>
        ...
        <div className="lg:col-span-3 space-y-8">
           <section className="glass-card p-6 border-zinc-800">
             <div className="flex flex-col gap-1 mb-6">
                <span className="text-[8px] font-black text-teal-500 uppercase tracking-[0.3em]">INFILTRATOR_NODE</span>
             ...
             <div className="space-y-4 border-t border-white/5 pt-6">
                <div className="flex justify-between items-center text-[10px]">
                   <span className="text-zinc-500 font-bold uppercase tracking-widest">PROTOCOL:</span>
                   <span className="text-teal-400 font-black">NEXUS_V1</span>
                </div>
                <div className="flex justify-between items-center text-[10px]">
                   <span className="text-zinc-500 font-bold uppercase tracking-widest">LANGUAGE:</span>
                   <span className="text-white font-black uppercase">{board.language}_SYNC</span>
                </div>
             </div>
           </section>
